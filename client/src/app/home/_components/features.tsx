    ...
    <div className="mt-5 space-y-2.5">
      {items.map((item, i) => (
        <div
          ...
          style={{ opacity: active ? 1 : 0, transform: active ? "translateX(0)" : "translateX(12px)", transition: `all 0.5s ease ${i * 130}ms` }}
        >
          <div className={`h-7 w-7 rounded-lg flex items-center justify-center shrink-0 transition-colors duration-500 ${
            animated[i] >= 100 ? "bg-primary/15 text-primary" : "bg-gray-100 text-gray-400"
          }`}>
            {animated[i] >= 100 ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Target className="w-3.5 h-3.5" />}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-medium text-gray-700 truncate">{item.label}</span>
              <span className="text-[10px] text-gray-400 shrink-0 ml-2 tabular-nums">{animated[i]}%</span>
            </div>
            <div className="h-1 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full bg-primary/60 rounded-full transition-all duration-300" style={{ width: `${animated[i]}%` }} />
            </div>
          </div>
        </div>
      ))}
      ...
      >
        {[
          ...
          <div key={stat.label} className="rounded-lg bg-gray-50 border border-gray-100 px-2 py-1.5 text-center">
    ...
    <div className="mt-5 space-y-2">
      {rows.map((item, i) => (
        <div
          ...
          style={{ opacity: active ? 1 : 0, transform: active ? "translateY(0)" : "translateY(8px)", transition: `all 0.4s ease ${i * 200}ms` }}
        >
          ...
          {item.score && (
            <span className="text-xs font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-md animate-[score-pop_0.4s_ease_forwards]"
...
function SupportVisual({ active }: { active: boolean }) {
  const messages = [
    ...
    { from: "agent", text: "Go to Settings \u2192 API Keys \u2192 Regenerate. Your old key will be revoked immediately." },
  ];
  return (
    <div className="mt-5 space-y-2">
      {messages.map((msg, i) => {
        ...
        return (
          <div
            ...
            style={{ opacity: active ? 1 : 0, transform: active ? "scale(1)" : "scale(0.92)", transition: `all 0.4s ease ${i * 350}ms` }}
          >
      ...
      >
        <div className="flex -space-x-1">
          {["WA", "TG", "SL", "DC"].map((ch) => (
            <div key={ch} className="w-5 h-5 rounded-full bg-gray-100 border-2 border-white flex items-center justify-center">
        ...
        {nodes.map((node, i) => (
          <div key={node.label} className="flex items-center">
            <div
              ...
              style={{ opacity: active ? 1 : 0, transform: active ? "scale(1)" : "scale(0.8)", transition: `all 0.4s ease ${i * 180}ms` }}
            >
              {node.label}
            </div>
            {i < nodes.length - 1 && (
              <div className="relative w-4 sm:w-6 h-px overflow-visible">
                <div
                  ...
                  style={{ transform: active ? "scaleX(1)" : "scaleX(0)", transition: `transform 0.3s ease ${i * 180 + 200}ms` }}
                />
                <div
                  className="absolute -right-0.5 -top-[3px] w-0 h-0 border-l-[4px] border-l-gray-300 border-y-[3px] border-y-transparent"
                  style={{ opacity: active ? 1 : 0, transition: `opacity 0.2s ease ${i * 180 + 350}ms` }}
                />
                {active && (
                  <div
                    className="absolute top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-primary"
                    style={{ animation: `packet-flow-short 1.2s ease-in-out infinite ${i * 300}ms` }}
                  />
                )}
              </div>
            )}
      ...
      <div className="flex flex-wrap gap-1.5">
        {apps.map((app, i) => (
          <div
            ...
            style={{ opacity: active ? 1 : 0, transform: active ? "translateY(0)" : "translateY(6px)", transition: `all 0.3s ease ${i * 80}ms` }}
          >
            {app.name}
          </div>
        ))}
...
function LoyaltyVisual({ active }: { active: boolean }) {
  const tiers = [
    { name: "Bronze", icon: Trophy, points: 500, color: "text-amber-700 bg-amber-50 border-amber-200", bar: "bg-amber-400", filled: true },
    { name: "Silver", icon: Star, points: 1200, color: "text-slate-600 bg-slate-50 border-slate-200", bar: "bg-slate-400", filled: true },
    { name: "Gold", icon: Zap, points: 3000, color: "text-yellow-600 bg-yellow-50 border-yellow-200", bar: "bg-yellow-400", filled: false },
  ];
  ...
  return (
    <div className="mt-5 space-y-3">
      ...
      <div
        ...
        style={{ opacity: active ? 1 : 0, transform: active ? "translateY(0)" : "translateY(8px)", transition: "all 0.4s ease 0ms" }}
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-[10px] font-bold text-primary tabular-nums">{currentPoints.toLocaleString()} pts</span>
          </div>
          ...
          <p className="text-[10px] text-gray-400 mt-0.5">{maxPoints - currentPoints} pts to Gold tier</p>
        </div>
      </div>
        ...
        {tiers.map((tier, i) => {
          ...
          return (
            <div
              ...
              style={{ opacity: active ? 1 : 0, transform: active ? "scale(1)" : "scale(0.85)", transition: `all 0.4s cubic-bezier(0.16,1,0.3,1) ${i * 120 + 200}ms` }}
            >
      ...
      >
        ...
        <span className="text-[10px] text-gray-400 font-medium">Loyalty pass active · invite link ready</span>
      </div>
    </div>
  );
}
...
function useCardHover() {
  ...
  return { hovered, handlers: { onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false) } };
}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="flex items-start gap-4 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-base font-semibold text-gray-900 tracking-tight">Loyalty Infrastructure</h3>
                      <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-bold tracking-wide">AUTOMATED</span>
                    </div>
                    <p className="text-sm text-gray-500 leading-relaxed">
                      Launch whitelabel loyalty programs with tier-based digital passes. Define tiers, automate
                      point gifting, and send invite links to members.
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 mt-4">
                  {["Create programs", "Issue loyalty passes", "Tier-based rewards", "Bulk invite links", "AI-automated gifting", "Vouchers & deals"].map((tag) => (
                    <span key={tag} className="px-2.5 py-1 rounded-lg bg-gray-50 border border-gray-200 text-[11px] font-medium text-gray-600 hover:border-primary/30 hover:bg-primary/5 transition-colors cursor-default">
                      {tag}
                    </span>
                  ))}
                </div>
              </div>
              <LoyaltyVisual active={loyalty.hovered || !section.visible ? loyalty.hovered : true} />
            </div>
          </div>
            ...
            <div className="grid sm:grid-cols-2 gap-6 items-start">
              <div>
                <div className="flex items-start gap-4 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-semibold text-gray-900 tracking-tight mb-1">AI Goal Orchestration</h3>
                    <p className="text-sm text-gray-500 leading-relaxed">
                      Set high-level objectives. AI decomposes them into sub-tasks, coordinates sub-agents,
                      tracks progress, and self-corrects through reflection — automatically.
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 mt-4">
                  {["Set objectives", "Sub-task decomposition", "Agent coordination", "Self-reflection", "Progress tracking", "Auto-reporting"].map((tag) => (
                    <span key={tag} className="px-2.5 py-1 rounded-lg bg-gray-50 border border-gray-200 text-[11px] font-medium text-gray-600 hover:border-primary/30 hover:bg-primary/5 transition-colors cursor-default">
                      {tag}
                    </span>
                  ))}
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-base font-semibold text-gray-900 tracking-tight">Support Agents</h3>
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-base font-semibold text-gray-900 tracking-tight">Visual Workflow Builder</h3>
                <p className="text-sm text-gray-500 mt-1 leading-relaxed">
                  Build automations by connecting triggers, actions, conditions, and AI models
                  on a drag-and-drop canvas. Run on schedules or events.
                </p>
              </div>
            </div>
            <WorkflowVisual active={workflow.hovered || !section.visible ? workflow.hovered : true} />
          </div>
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-base font-semibold text-gray-900 tracking-tight">Task Compliance</h3>
                <p className="text-sm text-gray-500 mt-1 leading-relaxed">
            ...
            <div className="grid sm:grid-cols-2 gap-6 items-start">
              <div>
                <div className="flex items-start gap-4 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-semibold text-gray-900 tracking-tight mb-1">800+ App Integrations</h3>
                    <p className="text-sm text-gray-500 leading-relaxed">
                      Powered by Composio. Connect to Google Docs, Sheets, Notion, GitHub, Jira, HubSpot,
                      Salesforce, and more. Deliver reports anywhere your team works.
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 mt-4">
                  {["Google Docs", "Sheets", "Notion", "Slack", "Jira", "HubSpot", "GitHub", "+793 more"].map((tag) => (
                    <span key={tag} className="px-2.5 py-1 rounded-lg bg-gray-50 border border-gray-200 text-[11px] font-medium text-gray-600 hover:border-sky-300 hover:bg-sky-50 transition-colors cursor-default">
                      {tag}
                    </span>
                  ))}
                </div>
              </div>
              <IntegrationVisual active={integration.hovered || !section.visible ? integration.hovered : true} />
            </div>
          </div>
