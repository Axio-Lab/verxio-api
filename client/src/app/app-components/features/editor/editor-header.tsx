...
export const EditorSaveButton = ({ workflowId }: { workflowId: string }) => {
  ...
  const checkForChanges = useCallback(() => {
    ...
    const currentNodesFiltered = currentNodes.filter(
      (node) => node.type !== "INITIAL" && node.type !== NodeType.INITIAL
    );
    ...
    const currentEdgesForCompare = currentEdges
      .map((edge) => ({
        source: edge.source,
        target: edge.target,
    ...
    const savedConnectionsForCompare = (workflow.connections || [])
      .map((conn) => ({
        source: conn.source,
        target: conn.target,
