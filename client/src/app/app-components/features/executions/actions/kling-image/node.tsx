...
import { KlingImageDialog, KlingImageFormValues } from "./dialog";
import { useNodeStatus } from "../../hooks/use-node-status";
import { useReactFlow } from "@xyflow/react";
import { Palette } from "lucide-react";

export const KlingImageNode = memo((props: NodeProps) => {
  ...
  const handleSubmit = (values: KlingImageFormValues) => {
    setNodes((nodes) =>
      nodes.map((node) =>
  ...
  const description =
    (nodeData?.prompt as string)?.length > 50
      ...
      : (nodeData?.prompt as string) || "Configure Kling Image";
  ...
  return (
    <>
      <KlingImageDialog
        ...
        defaultValues={nodeData as Partial<KlingImageFormValues>}
      />
      <BaseExecutionNode
        {...props}
        icon={Palette}
...
KlingImageNode.displayName = "KlingImageNode";
