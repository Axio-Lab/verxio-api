...
export const ValyuDeepResearchNode = memo((props: NodeProps) => {
  ...
  const getDescription = () => {
    ...
    return "Comprehensive AI research";
  };
  ...
  const handleSubmit = (values: ValyuDeepResearchFormValues) => {
    setNodes((nodes) =>
      nodes.map((node) => {
        if (node.id === props.id) {
          return {
  ...
  return (
    <>
      ...
      <BaseExecutionNode
        {...props}
        icon="/logo/valyu.svg"
        name={nodeData?.label || "Valyu Deep Research"}
...
ValyuDeepResearchNode.displayName = "ValyuDeepResearchNode";
