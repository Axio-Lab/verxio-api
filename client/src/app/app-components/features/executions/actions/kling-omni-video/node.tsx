...
import { KlingOmniVideoDialog, KlingOmniVideoFormValues } from "./dialog";
import { useNodeStatus } from "../../hooks/use-node-status";
import { useReactFlow } from "@xyflow/react";
import { Video } from "lucide-react";

export const KlingOmniVideoNode = memo((props: NodeProps) => {
  const [dialogOpen, setDialogOpen] = useState(false);
  const { setNodes } = useReactFlow();
  const { status: nodeStatus, output } = useNodeStatus({ nodeId: props.id });

  const handleOpenSettings = () => setDialogOpen(true);
  const handleSubmit = (values: KlingOmniVideoFormValues) => {
    setNodes((nodes) =>
      nodes.map((node) =>
  ...
  const description =
    (nodeData?.prompt as string)?.length > 50
      ...
      : (nodeData?.prompt as string) || "Configure Kling Omni Video";

  return (
    <>
      <KlingOmniVideoDialog
        open={dialogOpen}
        onOpenChange={setDialogOpen}
        onSubmit={handleSubmit}
        defaultValues={nodeData as Partial<KlingOmniVideoFormValues>}
      />
      <BaseExecutionNode
        {...props}
        icon={Video}
...
KlingOmniVideoNode.displayName = "KlingOmniVideoNode";
