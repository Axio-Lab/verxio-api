...
export const httpTriggerExecutor: NodeExecutor<HttpTriggerData> = async ({
  ...
  try {
    ...
    await validateAndPublishError(publish, nodeId, !data.variables, "Variable name is required");
    await validateAndPublishError(publish, nodeId, !data.method, "HTTP Method is not configured");
