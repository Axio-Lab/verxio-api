...
export const googleDriveExecutor: NodeExecutor<GoogleDriveData> = async ({
  ...
  try {
    ...
    if (!data.credentialId) {
      ...
      const error = new NonRetriableError("Google Drive node: Google OAuth credential is required");
      await publish(
    ...
    if (!credential) {
      await publishStatus(publish, nodeId, "error");
      const error = new NonRetriableError("Google Drive node: Credential not found");
      await publish(
        googleDriveChannel().output({
          nodeId,
          output: {
    ...
    if (credential.type !== CredentialType.GOOGLE_OAUTH) {
      ...
      const error = new NonRetriableError(
        "Google Drive node: Credential type mismatch. Expected GOOGLE_OAUTH credential."
      );
      await publish(
        googleDriveChannel().output({
          nodeId,
