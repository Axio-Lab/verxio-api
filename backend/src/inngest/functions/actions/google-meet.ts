...
export const googleMeetExecutor: NodeExecutor<GoogleMeetData> = async ({
  ...
  try {
    ...
    if (!data.credentialId) {
      ...
      const error = new NonRetriableError("Google Meet node: Google OAuth credential is required");
      await publish(
    ...
    if (!credential) {
      await publishStatus(publish, nodeId, "error");
      const error = new NonRetriableError("Google Meet node: Credential not found");
      await publish(
        googleMeetChannel().output({
          nodeId,
          output: {
    ...
    if (credential.type !== CredentialType.GOOGLE_OAUTH) {
      ...
      const error = new NonRetriableError(
        "Google Meet node: Credential type mismatch. Expected GOOGLE_OAUTH credential."
      );
      await publish(
        googleMeetChannel().output({
          nodeId,
