...
export const gmailExecutor: NodeExecutor<GmailData> = async ({
  ...
  try {
    ...
    if (!data.credentialId) {
      ...
      const error = new NonRetriableError("Gmail node: Google OAuth credential is required");
      await publish(
        gmailChannel().output({
          nodeId,
          output: {
    ...
    if (!credential) {
      await publishStatus(publish, nodeId, "error");
      const error = new NonRetriableError("Gmail node: Credential not found");
      await publish(
        gmailChannel().output({
          nodeId,
          output: {
    ...
    if (credential.type !== CredentialType.GOOGLE_OAUTH) {
      ...
      const error = new NonRetriableError(
        "Gmail node: Credential type mismatch. Expected GOOGLE_OAUTH credential."
      );
      await publish(
        gmailChannel().output({
          nodeId,
