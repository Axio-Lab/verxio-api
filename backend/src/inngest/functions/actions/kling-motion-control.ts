...
export const klingMotionControlExecutor: NodeExecutor<KlingMotionControlData> = async ({
  ...
  try {
    ...
    await checkNodeAccess(userId, "KLING_MOTION_CONTROL");
    ...
    try {
      await step.run(`kling-motion-control-consume-quota-${nodeId}`, async () => {
        await consumePremiumQuota(userId, QUOTA_COST.KLING_MOTION_CONTROL);
      ...
      await step.run(`kling-motion-control-quota-err-${nodeId}`, async () => {
        await publish(
          klingChannel().output({
            nodeId,
