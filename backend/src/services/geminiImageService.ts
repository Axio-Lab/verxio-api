...
/**
 * Multi-turn image editing conversation
 * Maintains context across multiple edits
 */
export async function multiTurnEdit(options: {
  ...
  aspectRatio?: AspectRatio;
  imageSize?: ImageSize;
}): Promise<GenerateImageResult & { updatedHistory: ConversationMessage[] }> {
  try {
    const ai = getGeminiClient();
    const model = options.model || "gemini-2.5-flash-image";
    ...
    // Add new message
    const newParts: any[] = [];
    if (options.newImageBase64) {
      newParts.push({
        inlineData: {
          mimeType: options.newImageMimeType || "image/png",
          data: options.newImageBase64,
        },
      });
    }
    newParts.push({ text: options.newMessage });
    ...
    const updatedHistory: ConversationMessage[] = [
      ...
      {
        role: "user",
        content: options.newMessage,
        imageBase64: options.newImageBase64,
        imageMimeType: options.newImageMimeType,
      },
      {
        role: "model",
        content: text || "",
