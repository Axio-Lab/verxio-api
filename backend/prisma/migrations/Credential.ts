...
import type { PrismaClient, Credential } from '../../../client/node_modules/.prisma/client'
import {
...
export async function migrate(
  ...
  return visitRecords<PrismaClient, Cursor>({
    ...
    client,
    getTotalCount: client.credential.count,
    migrateRecord,
    reportProgress,
  })
}
...
async function migrateRecord(client: PrismaClient, cursor: Cursor | undefined) {
  return await client.$transaction(async tx => {
    const record = await tx.credential.findFirst({
      take: 1,
